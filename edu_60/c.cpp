...
int main()
{
    ...
    // for(auto a : move) cout << a.F << " "<< a.S << endl;
    ...
    while(mx - mn > 1) {
       ...
       //cout << mn << " " << mx << " "<< mid << " c "<< cycle << " " << md << endl;
        
        LLI x3 = move[0].F + cycle * (move[n].F-move[0].F) + (move[md].F-move[0].F);
        LLI y3 = move[0].S + cycle * (move[n].S-move[0].S) + (move[md].S-move[0].S);
        
       // cout << "xy " << " "<< x3 << " " << y3 << endl;
        if(abs(x3-x2) + abs(y3-y2) <= mid) {
        //    cout << x3 << " "<< y3 << " "<< x2 << " " << y2 << " "<< mid << endl;
            ans = mid;
            mx = mid;
        }else {
            mn = mid;
