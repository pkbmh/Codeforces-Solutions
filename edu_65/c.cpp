...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    for(int gg = 1; gg <= m; ++gg) {
        ...
        for(int i = 0; i < gs; ++i) {
            cin >> arr[i];
            if(gid_mp.count(arr[i])) {
                ids.insert(gid_mp[arr[i]]);
                if(gset[fid].size() < gset[gid_mp[arr[i]]].size()) {
                    fid = gid_mp[arr[i]];
                }
            }
        }
        for(int i = 0; i < gs; ++i) {
            gset[fid].insert(arr[i]);
            gid_mp[arr[i]] = fid;
        }
        for(int i : ids) {
            if(i != fid){   
    ...
    for(int i = 1; i <= n; ++i) {
        if(gid_mp.count(i) == 0) cout << 1 << " " ;
        else cout << gset[gid_mp[i]].size() << " " ;
    }
    cout << endl;
return 0;
}
