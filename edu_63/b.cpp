...
bool foo(string &str, int f, int d, int i, int j) {
    if(d == 0) {
        int k = 0;
        while(k < str.length() && str[k] =='a') k++;
        return str[k] == '8';    
    }

    if(f) {
        str[i] = 'a';
        while(i < str.length() && (str[i] == '8' || str[i] == 'a')) i++;
        if(i == str.length()){
            i = 0;
            while(i < str.length() && str[i] == 'a') i++;
        }
        return foo(str, 1-f, d-1, i, j);
    }else {
        str[j] = 'a';
        while(j < str.length() && str[j] != '8') j++;
        if(j == str.length()) {
            j = 0;
...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    string str;
    cin >> str;
    int cnt = count(ALL(str), '8');
    if(cnt == n) {cout << "YES" << endl; return 0;}
    if(cnt <= (n-11)/2) {cout << "NO" << endl; return 0;}
    int i = 0;
    while(i < n && str[i] == '8') i++;
    int j = 0;
    while(j < n && str[j] != '8') j++;

    if(foo(str, 1, n-11, i, j)) cout << "YES" << endl;
    else cout << "NO" << endl;
return 0;
}
