...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    for(int i = 1; i < n;++i) {
                   // 0 0                       1 2                           2 1
        dp[i][0] = (dp[i-1][0] * x) % MOD7 + (dp[i-1][1] * y) % MOD7 + (dp[i-1][2] * z) % MOD7;
        
                    // 0 1                       1 0                        2 2
        dp[i][1] = (dp[i-1][0] * z) % MOD7 + (dp[i-1][1] * x) % MOD7 + (dp[i-1][2] * y) % MOD7;

                   // 0 2                         2 0                       1 1
        dp[i][2] = (dp[i-1][0] * y) % MOD7 + (dp[i-1][2] * x) % MOD7 + (dp[i-1][1] * z) % MOD7;
        
    }
    cout << dp[n-1][0]%MOD7 << endl;
return 0;
}
