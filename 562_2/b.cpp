...
bool check_(VPII &arr, int a, int b) {
    for(auto i : arr) {
        if(i.F != a && i.F != b && i.S != a && i.S != b) return false;
    }
    return true;
}

bool foo(VPII &arr, int a) {
    for(auto p : arr) {
        if(a != p.F && a != p.S) {
            if (check_(arr, a, p.F) || check_(arr, a, p.S)) {
            return true;
            }else return false;
        }
    }
...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    if(foo(arr, arr[0].F) || foo(arr, arr[0].S)) cout << "YES" << endl;
    else cout << "NO" << endl;
    return 0;
}
