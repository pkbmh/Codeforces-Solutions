...
bool vis[MAX][6];
int n;
bool foo(int arr[], int ind, int pre = 0) {
    if(ind == n) return true;
    if(vis[ind][pre]) return false;

    if(ind == 0) {
        for(int i = 1; i <= 5; ++i) {
            ans[ind] = i;
            if(foo(arr, ind+1, i)) return true;
            ans[ind] = -1;
        }
    }else {
        if(arr[ind] > arr[ind-1]) {
            if(pre == 5) return false;
            else {
            ...
            for(int i = 1; i <= 5; ++i) {
                ans[ind] = i;
                if(pre != i && foo(arr, ind+1, i)) return true;
                ans[ind] = -1;
            }
        }
    }
...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    SETN1(ans);
    int arr[n];
    for(int i = 0; i <= 5; ++i) vis[0][i] = 0;
    for(int i = 0; i < n; ++i) {
        cin >> arr[i];
        /*
        for(int j = 1; j <= 5; ++j) {
            for(int k = 1; k <= 5; ++k) {
                if(arr[i] > arr[i-1] && k > j && res[i-1][j]){
    ...
    int f = foo(arr, 0);
    if(f) {for(int i = 0; i < n; ++i) cout << ans[i] << " "; cout << endl;}
    else cout << -1 << endl;
return 0;
}
