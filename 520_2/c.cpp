...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    arr[0] = {0,0};
    for(int i = 0; i < n; ++i) {
        arr[i+1] = {arr[i].F + (str[i] == '0'), arr[i].S + (str[i] == '1')};
    }
    while(q--) {
        cin >> l >> r;
        PLLI v  = {arr[r].F - arr[l-1].F, arr[r].S - arr[l-1].S};
        LLI sum_1 = PowMod(2LL, r-l+1, MOD7) + MOD7 - 1;
        sum_1 %= MOD7;
        
        LLI sum_0 = PowMod(2LL, v.F, MOD7) + MOD7 - 1;
        sum_0 %= MOD7;

        cout << (sum_1 + MOD7 - sum_0) % MOD7 << endl;
    }
