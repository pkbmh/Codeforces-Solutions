...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    LOOP(n) {
        ...
        f = mp[t];
        st[f+1]++;
        if (st[f] <= 1) st.erase(f);
        else st[f]--;

        if(st.size() == 1){
            if(st.begin()->second == i+1 || st.begin()->first == i+1) len = i+1;
            else if(st.begin()->second == 2)
                len = max(len, 2*st.begin()->first-1);
        }
        if (st.size() == 2) {   
            ...
            it++;
            auto b = *it;
            if(a.F == 1 && a.S == 1) len = i + 1;
            else if(b.F == 1 && b.S == 1) len = i + 1;
            else if(1 == abs(a.F - b.F)) {
                if(a.F > b.F && a.S == 1) len = i+1;
                else if(b.F > a.F && b.S == 1) len = i+1;
            }
        }
