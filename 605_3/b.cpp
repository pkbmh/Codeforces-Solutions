...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    while(t--) {
        cin >> str;
        int cnt_r = count(ALL(str), 'R');
        int cnt_l = count(ALL(str), 'L');
        int cnt_u = count(ALL(str), 'U');
        int cnt_d = count(ALL(str), 'D');
        ...
        */
        ...
        int cnt = 2 * hz + 2 * vz;
        cout << cnt << endl;
        if(cnt > 0) cout << string(hz, 'L') << string(vz, 'U') << string(hz, 'R') << string(vz, 'D') << endl;
