...
int mp[MAX][2][2], arr[MAX];
int n;

int foo(int ind, bool isPre, bool isSkipped) {
    if(ind == n) return 0;
    if(mp[ind][isPre][isSkipped] != -1) return mp[ind][isPre][isSkipped];
    int i = ind-1;
    if( ! isPre ) i--;
    int mx = 0;
    if(arr[i] < arr[ind]) mx = foo(ind+1, true, isSkipped)+1;
    if(! isSkipped)  mx = max(mx, foo(ind+1, false, true));
    return mp[ind][isPre][isSkipped] = mx;
}
int main() {
ios::sync_with_stdio(0); cin.tie(0);
    ...
    int mx = 1;
    for(int i = 0; i < n-1; ++i) {
        mx = max(mx, foo(i+1, true, false)+1);
    }
    cout << mx << endl;
return 0;
}
