...
LLI solve(LLI l, LLI h, VLLI &pos, LLI a, LLI b) {
   
    int llb = lower_bound(pos.begin(), pos.end(), l) - pos.begin();
    int hlb = upper_bound(pos.begin(), pos.end(), h) - pos.begin();
    hlb--;
    if(hlb-llb+1 == 0)
        return a;
     
    LLI ans = (hlb-llb+1) * 1LL * b * (h-l+1);
    
    if(l == h) return ans;

    LLI mid = (l+h) / 2;
    
    LLI left = solve(l, mid, pos, a, b);
    LLI right = solve(mid+1, h, pos, a, b);
    
   // cout << "ans " << l << " "<< h << " "<< llb << " " << hlb << " " << ans << endl;
    // cout << "Left " << left << endl;
