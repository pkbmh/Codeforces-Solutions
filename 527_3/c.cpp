...
string is_cool(vector<PSI> &arr, string str) {
    string res = string(arr.size(), 'S'); 
    for(int i = 0; i < arr.size(); i+=2){
        string s = arr[i].F;
        string s1 = arr[i+1].F;

        string pre = str.substr(0, s.length());
        string suf = str.substr(str.length()-s.length(), s.length());
        if(s != pre && s != suf) return "";
        if(s1 != pre && s1 != suf) return "";
...
//ios::sync_with_stdio(0); cin.tie(0);
    int n;
    cin >> n ;
    vector<PSI> arr(2*n - 2);
    int t = 2 * n - 2;
    string s1 = "", s2 = "";
    for(int i = 0; i < t; ++i) {
        cin >> arr[i].F;
        arr[i].S = i;
        if(arr[i].F.length() == n-1) {
            if(s1.length() == 0) s1 = arr[i].F;
            else s2 = arr[i].F;
        }
    }
    sort(arr.begin(), arr.end(), [](const PSI &a, const PSI &b) {
                return a.F.length() < b.F.length();
            });
    string res = is_cool(arr, s1 + s2[n-2]);
    if(res.length() == t) cout << res << endl;
    else {
        cout << is_cool(arr, s2 + s1[n-2]) << endl;
    }
return 0;
}
