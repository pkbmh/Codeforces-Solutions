...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    for(int i = 0; i < n; ++i) 
        if(rows[i].size() == 1){
            top[i][rows[i][0]] = left[i];
            if(front[rows[i][0]] == left[i]) front[rows[i][0]] = -1;
            left[i] = -1;
        }

    for(int i = 0; i < m; ++i) {
        if(front[i] != -1 && cols[i].size() == 1) {
            top[cols[i][0]][i] = front[i];
            front[i] = -1;
        }
    }
