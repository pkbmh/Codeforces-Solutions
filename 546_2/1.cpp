...
int n;
	cin >> n;
    	typedef std::pair<int, int> myPair; // typedef to shorten the type name
	...
	cin >> k;
    	...
    	myPair cmp(k, 0); // reference value (set this up as you want)
    	int ind= std::lower_bound(vec.begin(), vec.end(), cmp, 
        	[](myPair lhs, myPair rhs) -> bool { return lhs.first < rhs.first; }) - vec.begin();

	if(ind > 0 && vec[ind-1].second >= k) {
		ind--;
