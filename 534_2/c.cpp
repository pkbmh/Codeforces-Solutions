...
//ios::sync_with_stdio(0); cin.tie(0);
    vector<string > mat = {"0000", "0000", "0000", "0000"};
    string str;
    cin >> str;
    bool f1, f2;
    f1 = f2 = true;
    ...
    /*for(int k = 0; k < str.length(); ++k ) {
        ...
        for(int i = 0; !flag && i < 4; ++i) {
            for(int j = 0; !flag && j < 4; ++j) {
                if(c == '0') {
                    if(mat[i][j] == '0' && i < 3 && mat[i+1][j] == '0') {
                        mat[i][j] = mat[i+1][j] = '1';
                        cout << i+1 << " "<< j + 1 << "\n";
                        flag = true;
                    }
                }else {
                    if(mat[i][j] == '0' && j < 3 && mat[i][j+1] == '0') {
                        mat[i][j] = mat[i][j+1] = '1';
                        cout << i+1 << " "<< j + 1 << "\n";
                        flag = true;
                    }
        ...
        VI rows(4, 0), cols(4, 0);
        for(int i = 0; i < 4; ++i) {
            int j = 0;
            for(j = 0; j < 4; ++j) if(mat[i][j] != '1') break;
            if(j == 4) rows[i] = 1; //for(j = 0; j < 4; ++j) mat[i][j] = '0';
        
            j = 0;
            for(j = 0; j < 4; ++j) if(mat[j][i] != '1') break;
            if(j == 4) cols[i] = 1; //for(j = 0; j < 4; ++j) mat[j][i] = '0';
        }
        for(int i = 0; i < 4; ++i) {
            if(rows[i])for(int j = 0; j < 4; ++j) mat[i][j] = '0';
            if(cols[i])for(int j = 0; j < 4; ++j) mat[j][i] = '0';
