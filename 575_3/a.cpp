...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    while(t--) {
        ...
        if (arr[0] + arr[1] == arr[2]) cout << arr[2] << endl;
            ...
            // split 0
            LLI diff = arr[2] - arr[1];
            LLI ans0 = arr[1] + min(diff, arr[0]) + max(0LL, arr[0]-diff) / 2;
            
            diff = arr[2] - arr[0];
            LLI ans1 = arr[0] + min(diff, arr[1]) + max(0LL, arr[1]-diff) / 2;

            diff = arr[1] - arr[0];
            LLI ans2 = arr[0] + min(diff, arr[2]) + max(0LL, arr[2]-diff) / 2;
