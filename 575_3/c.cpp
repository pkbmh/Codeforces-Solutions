...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    while(t--) {
        cin >> n;
        node arr[n];
        LOOP(n) cin >> arr[i].x >> arr[i].y >> arr[i].l >> arr[i].u >> arr[i].r >> arr[i].d;
        sort(arr, arr+n, [](const node &f, const node &s) {
                if(f.x != s.x) return f.x < s.x;
                return f.y < s.y;
                });
        int min_x = -1e5, max_x = 1e5, min_y = -1e5, max_y = 1e5;
        LOOP(n) {
            if(arr[i].l == 0) min_x = max(min_x, arr[i].x);
            if(arr[i].r == 0) max_x = min(max_x, arr[i].x);
            if(arr[i].u == 0) max_y = min(max_y, arr[i].y);
            if(arr[i].d == 0) min_y = max(min_y, arr[i].y);
        }
