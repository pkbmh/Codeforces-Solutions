...
int main()
{
    ...
    for(int i = 0; i < n; ++i) {
        ...
        if(topmk.size() < m*k || *topmk.begin() < arr[i]) topmk.insert(arr[i]);
        
        if(topmk.size() > m*k) topmk.erase(topmk.begin());
    }
    int cnt = 0;
    VI ans;
    multiset<int> curr;
    ...
    for(int i = 0; k && i < n; ++i) {
        curr.insert(arr[i]);
        if(topmk.count(arr[i])) {
            cnt++;
            //curr.insert(arr[i]);
            //cout << "in " << arr[i] << endl;
            topmk.erase(topmk.find(arr[i]));
        }

        if(cnt == m || (n-(i+1)) == m*(k-1)) {
            k--;
            ans.push_back(i+1);
            cnt = 0;
            set<int>::reverse_iterator it = curr.rbegin();
            for(int j = 0; j < m; ++j) {
                    sum += *it;
              //      cout << *it << " its " << endl; 
                    it++;
