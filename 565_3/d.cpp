...
void get() {
    vis[0] = vis[1] = true;
    int i = 4;
    while(i < mx) {vis[i] = true; lst[i] = 2; i = i + 2;}
    prime[1] = 2;
    prime_r[2] = 1;
    pri.PB(2);
    int cnt = 2;
...
int get_id(int x) {
    ...
    for(int a : pri) if(x%a == 0) return max(a, x/a);
}
int get_id1(int x) {
    if(!vis[x]) return 1;
    return x/lst[x];
...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    while(st.size() > 0){
        ...
        if(!vis[a]) {
            ans.PB(prime_r[a]);
            st.erase(st.find(a));
            st.erase(st.find(prime_r[a]));
        }else {
            ans.PB(a);
            st.erase(st.find(a));
            st.erase(st.find(ld[a]));
        }
    }
