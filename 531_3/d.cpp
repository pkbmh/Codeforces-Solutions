...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    int need = n/3;
    map<char, int> skips;// = {{'0', 0}, {'1', 0}, {'2', 0}};

    for(int i = 0; i < n; ++i) {
        if(cnt['0'] < need && str[i] != '0') {
            if(str[i] == '1' && cnt['1'] > need) {
                str[i] = '0';
                cnt['0']++;
                cnt['1']--;
            }else if(str[i] == '2' && cnt['2'] > need) {
                str[i] = '0';
                cnt['0']++;
                cnt['2']--;
            }
        }else if(cnt['1'] < need && str[i] != '1') {
            if(str[i] == '0' && cnt['0'] > need) {
                if(skips['0'] == need) {
                    str[i] = '1';
                    cnt['1']++;
                    cnt['0']--;
