...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    for(int i = 1; i <= n; ++i) {
        ...
        if(arr[i-1][0] == MP(0,0) && arr[i-1][1] == MP(0,0)) continue;
        
        PII bt = {0,0};
        if(arr[i-1][0].F > 0) bt = {arr[i-1][0].F-1, max(oa, arr[i-1][0].S+t)};
        else if(arr[i-1][0].S > 0) bt = {arr[i-1][0].F, arr[i-1][0].S-1};

        PII bt1 = {0,0};
        if(arr[i-1][1].F > 0) bt1 = {arr[i-1][1].F-1, max(oa, arr[i-1][1].S+t)};
        else if(arr[i-1][1].S > 0) bt1 = {arr[i-1][1].F, arr[i-1][1].S-1};
        
        if(bt.F + bt.S  >= bt1.F + bt1.F)
            arr[i][0] = bt;
        else arr[i][0] = bt1;

    
        bt = {0,0};
        if(arr[i-1][0].S > 0) bt = {arr[i-1][0].F, arr[i-1][0].S-1};
        else if(arr[i-1][0].F > 0) bt = {arr[i-1][0].F-1, max(oa, arr[i-1][0].S+t)};
        
        bt1 = {0,0};
        if(arr[i-1][1].S > 0) bt1 = {arr[i-1][1].F, arr[i-1][1].S-1};
        else if(arr[i-1][1].F > 0) bt1 = {arr[i-1][1].F-1, max(oa, arr[i-1][1].S+t)};
