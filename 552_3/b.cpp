...
bool possible(VI &arr, int d, int val) {
    for(int i : arr) {
        if(i != val && i+d != val && i-d != val) return false;
    }
    return true;
}
int main() {
ios::sync_with_stdio(0); cin.tie(0);
    ...
    int diff = mx - mn;
    if(diff%2 == 0 && possible(arr, diff/2, mn+diff/2)) cout << diff/2 << endl;
    else if(possible(arr, diff, mn + diff)) cout << diff << endl;
    else cout << -1 << endl;
