...
bool is_sort(list<int> lst) {
    int pre = 0;
    for(int a: lst) {
        if(a != pre + 1) return false;
        pre++;
    }
    return true;
}
...
int get_(int n) {
    if(is_sort(lst)) return 0;
    if(za == 0) return n;
    int cnt = 0;
     while(true) {
        if(lst.back()+1 == *st.begin() || *st.begin() == 1) {
            lst.push_back(*st.begin());
            st.erase(st.begin());
            st.insert(lst.front());
            lst.pop_front();
        }else break;
...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    LOOP(n) {cin >> x; pos[x] = 0; st.insert(x); za = (x == 0 ? za+1 : za);}
    LOOP(n) {cin >> x; pos[x] = i+1; lst.push_back(x);}
    st.erase(0);
    int ans = 0;
    int ans1 = get_(n);
    
    LOOP(n) {
        ans = max(ans, pos[i+1]-i+n);
    }
