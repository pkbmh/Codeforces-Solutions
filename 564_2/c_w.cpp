...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    while(true) {
        if(lst.back()+1 == *st.begin()) {
            st.erase(lst.back()+1);
            lst.push_back(lst.back()+1);
            if(lst.front() == 0) za++;
            else st.insert(lst.front());
            lst.pop_front();
        }else {
            if(za > 0) {
                lst.push_back(0);
        ...
        for(int a: st) cout << a << " ";
        for(int i = 0; i < za; ++i) cout << 0 << " "; cout << endl;
        cout << "za " << za << " "<< lst.front() << " "<< lst.back() << endl;
        cin >> x;
        cnt++;
        if(za == n && lst.front() == 1 && lst.back() == n) break;
    }
    cout << cnt << endl;
