...
int main() {
    ...
    /* group will be having these members a members of type  
     * a ,  b,   c,  d
     * na   nb   n0  nab
     *
     * a+b+c+d = n/2      .. eq0
    ...
    for(int i = na; i >= 0; --i) {
        for(int j = 0; j <= nb; ++j) {
            ...
            d = d / 2;
            int c = (n/2) - (i+j+d);
            if(d >= 0 && d <= nab && c >= 0 && c <= n0 && i+d == (nb-j)+(nab-d)) {
               //cout << i <<  " " << j << " "<< c  << " " << d << endl;
                for(int k = 0; k < n; ++k) {
                     if(a[k] == '1' && b[k] == '1' && d) {cout << k+1 << " "; --d; }
                     else if(a[k] == '0' && b[k] == '0' && c) {cout << k+1 << " "; --c;}
                     else if(a[k] == '1' && b[k] == '0' && i){cout << k+1 << " "; --i;}
                     else if(b[k] == '1' && a[k] == '0' && j) {cout << k+1 << " "; --j;}
                }
                flag = true;
                goto stop;
            }
