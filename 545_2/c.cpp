...
int main() {
    ...
    int mat[n][m];
    VI rows[n], cols[m];
    FOR(i, 0, n-1) FOR(j, 0, m-1){ cin >> mat[i][j]; rows[i].PB(mat[i][j]); cols[j].PB(mat[i][j]);}
    ...
    FOR(i, 0, n-1) FOR(j, 0, m-1){
        ...
        int rless = lower_bound(rows[i].begin(), rows[i].end(), ele) - rows[i].begin();
        int rlarge = int(rows[i].size()) - rless - 1;
    
        int cless = lower_bound(cols[j].begin(), cols[j].end(), ele) - cols[j].begin();
        int clarge = int(cols[j].size()) - cless - 1;
        
        //cout << "i " << i << " " << j  << " rows " << rless << " "<< rlarge << " cols " << cless << " "<< clarge << endl;
        cout << max(rless, cless) + 1 + max(rlarge, clarge) << (j == m-1 ? '\n' : ' ');
    }
return 0;
}
