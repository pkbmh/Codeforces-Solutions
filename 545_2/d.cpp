...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    if(s.length() < t.length()) {
        ...
        string res = t;
        int lps_len = get_lps_len(t);
//        if(lps_len == 0) lps_len = t.length();

        while(res.length() != s.length()) {
            if(lps_len == 0)
                res += t.substr(0, min(t.length(), s.length()-res.length()));
            else res += t.substr(lps_len, min(t.length()-lps_len, s.length()-res.length()));
        }

        int szero = count(ALL(s), '0');

        int rzero = count(ALL(res), '0');
        
        for(int i = res.length()-1; i >= 0; --i){
            if(szero > rzero && res[i] == '1'){
                res[i] = '0';
                rzero++;
            }else if(szero < rzero && res[i] == '0') {
                res[i] = '1';
                rzero--;
            }

            if(szero == rzero) break;
        }
        cout << res << endl;
    }
