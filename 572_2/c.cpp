...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    LOOP(n) cin >> arr[i];
    map<PII, PII> mat;
    LOOP(n) mat[{1,i}] = {arr[i], 0};
    for(int l = 2; l <= n; l = l * 2) {
        for(int i = 0; i <= (n-l); ++i) {
            mat[{l,i}].F = mat[{l/2,i}].F + mat[{l/2,i+l/2}].F;
            mat[{l,i}].S = mat[{l/2,i}].S + mat[{l/2,i+l/2}].S;
            mat[{l,i}].S += (mat[{l,i}].F/10);
            mat[{l,i}].F %= 10;
            //cout << l << " "<< i << " " << mat[l][i].F << mat[l][i].S << endl;
        }
        //cout << "-----" << endl;
    }
    int q,l,r;
    ...
    while(q--) {
        ...
        int len = r-l+1;
        int i = l-1;
        int a = mat[{len/2,i}].F + mat[{len/2,i+len/2}].F;
        int b = mat[{len/2,i}].S + mat[{len/2,i+len/2}].S;
        cout << b + a/10 << endl;
    }
return 0;
}
