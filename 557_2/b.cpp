...
bool can() {
    // first row
    for(int i = 1; i < m; ++i) {
        if (a[0][i] <= a[0][i-1] || b[0][i] <= b[0][i-1]) return false;
    }
    // first col
    for(int i = 1; i < n; ++i) {
        if (a[i][0] <= a[i-1][0] || b[i][0] <= b[i-1][0]) return false;
    }
    for(int i = 1; i < n; ++i) {
        for(int j = 1; j < m; ++j) {
            if (a[i][j] <= a[i-1][j] || b[i][j] <= b[i-1][j] || a[i][j] <= a[i][j-1] || b[i][j] <= b[i][j-1]) return false;
        }
    }
    return true;
}
