...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    LOOP(m) {
        cin >> x;
        arr[x].mini = min(i+1, arr[x].mini);
        arr[x].maxi = max(i+1, arr[x].maxi);
        arr[x].v = true;
    }
    ...
    for(int i = 1; i <= n; ++i) {
        // going left
        if (i > 1) {
            if (arr[i].v && arr[i-1].maxi > arr[i].mini) arr[i].l = false;
        }
        if (i < n) {
            if(arr[i].v && arr[i+1].maxi > arr[i].mini) arr[i].r = false;
        }
        cnt = cnt + (!arr[i].v) + arr[i].l + arr[i].r;
    }
    cout <<  cnt << endl;
