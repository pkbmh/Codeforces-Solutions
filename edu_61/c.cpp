...
int main()
{
    ...
    gt1[0] = gt2[0] = 0;
    for(int i = 1; i <= n; ++i) {
        gt1[i] = gt1[i-1] + (cnt[i] >= 2 ? 1 : 0);
        gt2[i] = gt2[i-1] + (cnt[i] > 2 ? 1 : 0);
    }

    int mn = INT_MAX;
    for (int i = 0; i < q; i++) {
        for (int j = i+1; j < q; j++) {
            if(range[i].S < range[j].F) {
                int tmp = gt1[range[i].S] - gt1[range[i].F-1];
                tmp += gt1[range[j].S] - gt1[range[j].F-1];
                int total = range[i].S - range[i].F  + range[j].S - range[j].F  + 2;
                mn = min(mn, total-tmp);
            }else if(range[i].F <= range[j].F && range[j].S <= range[i].S){
                int tmp = gt1[range[j].F] - gt1[range[j].F - 1];
                tmp += gt1[range[j].S+1] - gt1[range[i].S-1];
            }else{
                int tmp = gt1[range[i].F] - gt1[range[j].F - 1];
                tmp += get1[range[S]]
            }
        }
    }
    cout << col - (mn1 + mn2) << endl;
return 0;
}
