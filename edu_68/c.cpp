...
bool can(string s, string d, multiset<char> &cs) {
    if(s == d) return true;
    if(d.length() <= s.length()) return false;

    if (s[0] == d[0]) return can(s.substr(1), d.substr(1), cs);
    if (cs.count(d[0]) == 0) return false;
    cs.erase(cs.find(d[0]));
    return can(s, d.substr(1), cs);
}
int main() {
ios::sync_with_stdio(0); cin.tie(0);
    ...
    while(t--) {
        ...
        for(char c1: c)
            cs.insert(c1);
        if (a == b || can(a, b, cs)) cout << "YES" << endl;
        else cout << "NO" << endl;
    }
